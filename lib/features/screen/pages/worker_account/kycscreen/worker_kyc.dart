...
//   String? _extractBlood(String s) =>
//       RegExp(r'\b(?:A|B|AB|O)[\+\-]\b', caseSensitive: false)
//           .firstMatch(s)
//           ?.group(0)
//           ?.toUpperCase();
...
//                   constraints.maxWidth > 900 ? 48.0 : 8.0;
...
//                             OutlinedButton(
//                               onPressed:
//                                   _loading ? null : details.onStepCancel,
...
/// =============================================================
/// KYC SERVICE  →  Firestore helpers (status + duplicate check)
/// =============================================================
class KycService {
  ...
  static Future<String> getKycStatus() async {
    ...
    // 1. Verified
    final verified = await _firestore.collection('verifyWorker').doc(uid).get();
    if (verified.exists) return 'verified';

    // 2. Rejected
    final rejected = await _firestore
        .collection('rejectWorkerVerification')
        .doc(uid)
        .get();
    if (rejected.exists) return 'rejected';

    // 3. Pending
    final pending = await _firestore.collection('workerKyc').doc(uid).get();
  ...
  /// Returns true if a pending KYC already exists
  static Future<bool> hasPendingKyc() async {
    final uid = _auth.currentUser!.uid;
    final doc = await _firestore.collection('workerKyc').doc(uid).get();
    return doc.exists;
  }
}

/// =============================================================
/// KYC STATUS CARD  →  shows Pending / Verified / Rejected / None
/// =============================================================
class KycStatusCard extends StatelessWidget {
  final String status;

  const KycStatusCard({super.key, required this.status});
    ...
    if (status == 'pending') {
      return Card(
        ...
        child: const ListTile(
          ...
          title: Text('KYC Status: Pending'),
          subtitle: Text('Your KYC is under review.'),
        ),
      );
    }
          ...
          child: Column(
            ...
            children: [
              const ListTile(
                ...
                title: Text('KYC Rejected'),
                subtitle: Text(
                  'Your KYC request was rejected.\nPlease submit your documents again.',
                ),
              ),
              SizedBox(
                width: 160,
...
/// =============================================================
/// KYC STATUS PAGE  →  uses KycService + KycStatusCard
/// =============================================================
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: FutureBuilder<String>(
        future: KycService.getKycStatus(),
        builder: (context, snapshot) {
          if (!snapshot.hasData) {
                ...
                children: [
                  ...
                  Text(
                    status == 'pending'
                        ...
                        : status == 'rejected'
                        ? 'You can submit your KYC documents again.'
...
class _KycWizardState extends State<KycWizard> {
  ...
  Future<void> _submit() async {
    ...
    // prevent double pending submission
    if (await KycService.hasPendingKyc()) {
      _snack('You already submitted a KYC request. Please wait for review.');
      return;
    }
